...
async function main(): Promise<void> {
  ...
  if (args.length === 1 && (args[0] === '--version' || args[0] === '-v' || args[0] === '-V')) {
    ...
    console.log(`${MACRO.VERSION} (Hone)`);
    return;
  }
  ...
  // perf-sensitive. No enableConfigs(), no analytics sinks at this layer ...  // workers are lean. If a worker kind needs configs/auth (assistant will),
  ...
  if (args[0] === 'loop') {
    const loopPresets: Record<string, { label: string; verify: string; stop: string; memory: string }> = {
      'health': {
        label: 'Project health check',
        verify: 'Run the repository build, type checks, backend checks, and focused smoke tests that match the files changed.',
        stop: 'Stop when all selected verification commands pass, or when the same blocker repeats twice with concrete evidence.',
        memory: 'Summarize changed files, commands run, pass/fail results, and residual risks.',
      },
      'tests': {
        label: 'Test reliability',
        verify: 'Run the failing or most relevant tests first, then broaden to the nearest full suite after the fix.',
        stop: 'Stop when the target test and nearest regression suite both pass, or when the failure is external and reproducible.',
        memory: 'Record failing test names, root cause, fix, and final test output.',
      },
      'mobile': {
        label: 'Mobile connectivity',
        verify: 'Check Gateway daemon status, local WebSocket, Relay auth, LAN address selection, and mobile pairing flow.',
        stop: 'Stop when pairing succeeds through the intended path, or when the remaining failure is an environment requirement.',
        memory: 'Record host/port, relay URL, close codes, firewall/LAN assumptions, and user-facing next action.',
      },
    }
    const subcmd = args[1]
    if (!subcmd || subcmd === 'list') {
      console.log('Hone Loop presets:')
      for (const [name, preset] of Object.entries(loopPresets)) {
        console.log(`  ${name.padEnd(8)} ${preset.label}`)
      }
      console.log('')
      console.log('Usage: hone loop <health|tests|mobile> [goal text]')
      return
    }
    const preset = loopPresets[subcmd]
    if (!preset) {
      console.error(`Unknown loop preset: ${subcmd}`)
      console.error('Usage: hone loop <health|tests|mobile> [goal text]')
      process.exit(1)
    }
    const goal = args.slice(2).join(' ').trim() || preset.label
    console.log([
      `Loop: ${preset.label}`,
      '',
      `Goal: ${goal}`,
      '',
      'Steps:',
      '1. Inspect the current state and identify the smallest actionable gap.',
      '2. Make the smallest scoped change that can move the goal forward.',
      `3. Verify: ${preset.verify}`,
      '4. If verification fails, use the new evidence to choose the next smallest fix and repeat.',
      '',
      `Stopping rule: ${preset.stop}`,
      `Memory/report: ${preset.memory}`,
    ].join('\n'))
    return
  }
  ...
  if (args[0] === 'pair') {
    ...
    const isBadMobileAddress = (addr: string): boolean => {
      const parts = addr.split('.').map(Number)
    ...
    const isVirtualInterface = (name: string): boolean => {
      return /vmware|virtualbox|vbox|hyper-v|wsl|docker|loopback|tailscale|zerotier|tap|tun|vpn/i.test(name)
    }

    // Determine the best LAN address to show the user. Mobile devices can only
    // reach the real Wi-Fi/Ethernet interface, not test/proxy/VM adapters.
    const nets = os.networkInterfaces()
    const candidates: Array<{ address: string; score: number }> = []
    for (const [name, ifaces] of Object.entries(nets)) {
      if (!ifaces) continue
      for (const ni of ifaces) {
        if (ni.family !== 'IPv4' || ni.internal || isBadMobileAddress(ni.address)) {
          continue
        }

        let score = 0
        if (/wi-?fi|wlan|wireless/i.test(name)) score += 60
        if (/ethernet|以太网/i.test(name)) score += 40
        if (isPrivateLanAddress(ni.address)) score += 30
        if (isVirtualInterface(name)) score -= 100
        candidates.push({ address: ni.address, score })
      }
    }
    candidates.sort((a, b) => b.score - a.score)
    const lanAddr = candidates[0]?.address ?? null
  ...
  // 获取或创建持久化的 machineId，保证 gateway 重启后身份不变（已配对客户端不会失联）
  async function getOrCreateMachineId(): Promise<string> {
    const fs = await import('fs/promises')
    const path = await import('path')
    const home = process.env.HOME || process.env.USERPROFILE || '.'
    const dataDir = process.env.HONE_DATA_DIR || path.join(home, '.hone')
    const idFile = path.join(dataDir, 'machine-id')
    try {
      const existing = await fs.readFile(idFile, 'utf-8')
      if (existing.trim()) return existing.trim()
    } catch { /* not yet created */ }
    const crypto = await import('crypto')
    const id = crypto.randomUUID()
    await fs.mkdir(path.dirname(idFile), { recursive: true }).catch(() => {})
    await fs.writeFile(idFile, id).catch(() => {})
    return id
  }
  ...
  if (args[0] === 'gateway') {
    ...
    if (subcmd === 'stop') {
      try {
        ...
        if (pid) {
          let alive = false
          try { process.kill(Number(pid), 0); alive = true } catch { /* stale PID */ }
          if (alive) {
            process.kill(Number(pid), 'SIGTERM')
            console.log(`Gateway 已停止 (PID: ${pid})`)
          } else {
            console.log('Gateway 未运行（已清理残留 PID 文件）')
          }
      ...
      try {
        ...
        if (pid) {
          ...
          if (alive) {
            ...
            console.log(`  PID: ${pid}`)
            console.log(`  中继: ${process.env.HONE_RELAY_URL || 'wss://hone-relay.marsailleippi79.workers.dev/connect'}`)
          } else {
            console.log('Gateway 未运行 (PID 文件残留，进程已退出)')
            await fs.unlink(pidFile).catch(() => {})
          }
        } else {
          console.log('Gateway 未运行')
    ...
    if (subcmd === 'approve' || subcmd === 'deny') {
      ...
      // 校验 clientId 仅含安全字符，防止路径遍历（如 ../../）
      if (!/^[a-zA-Z0-9_-]+$/.test(clientId)) {
        console.error(`无效的 clientId：仅允许字母、数字、下划线、连字符`)
        process.exit(1)
      }
      try {
        const fs = await import('fs/promises')
    ...
    // Check if already running
      try {
        const fs = await import('fs/promises')
        const existingPid = await fs.readFile(pidFile, 'utf-8').catch(() => null)
        if (existingPid) {
          try { process.kill(Number(existingPid), 0); console.log(`Gateway 已在运行 (PID: ${existingPid})`); process.exit(0) } catch { /* stale */ }
        }
      } catch {}

      console.log(`Hone Gateway 启动中...`)
      console.log(`  机器: ${machineName}`)
      console.log(`  中继: ${relayUrl}`)

      const { startGateway, stopGateway } = await import('../daemon/gateway.js')

      try {
        const fs = await import('fs/promises')
        const dir = process.env.HONE_DATA_DIR || `${process.env.HOME || process.env.USERPROFILE || '.'}/.hone`
  ...
  if (args.includes('--gateway-mode')) {
    ...
    const { stopGateway } = await import('../daemon/gateway.js')
