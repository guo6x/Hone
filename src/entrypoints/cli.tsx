...
async function main(): Promise<void> {
  ...
  if (args[0] === 'gateway') {
    const pidFile =
      ...
      `${process.env.HOME || '~'}/.hone/gateway.pid`
    const subcmd = args[1]
    ...
    if (subcmd === 'status') {
      try {
        ...
        const pid = await fs.readFile(pidFile, 'utf-8').catch(() => null)
        if (pid) {
          try { process.kill(Number(pid), 0) } catch { /* stale PID */ }
          // Check relay connectivity via Gateway (if running)
          console.log(`Gateway 运行中`)
          console.log(`  PID: ${pid}`)
          console.log(`  中继: ${process.env.HONE_RELAY_URL || 'wss://hone-relay.marsailleippi79.workers.dev/connect/default'}`)
        } else {
          console.log('Gateway 未运行')
        }
      } catch {
    ...
    if (!subcmd || subcmd === 'start') {
      ...
      const machineId = await import('crypto').then(m => m.randomUUID())
