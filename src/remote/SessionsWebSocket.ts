...
export class SessionsWebSocket {
  ...
  async connect(): Promise<void> {
    ...
    // Hone: use Hone relay instead of Anthropic API
    const relayUrl = process.env.HONE_RELAY_URL ||
    ...
    if (typeof Bun !== 'undefined') {
      ...
      ws.addEventListener('open', () => {
        logForDebugging(
          '[SessionsWebSocket] Connection opened, sending register',
        )
        // Send Hone register message
        try {
      ...
      ws.on('open', () => {
        ...
        // Hone: send register message after connect
        try {
          ws.send(jsonStringify({
            type: 'register',
            role: 'client',
