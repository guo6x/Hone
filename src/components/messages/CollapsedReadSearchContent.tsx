...
const teamMemCollapsed = false ? require('./teamMemCollapsed.js') as typeof import('./teamMemCollapsed.js') : null;
/* eslint-enable @typescript-eslint/no-require-imports */

// Hold each 猡?hint for a minimum duration so fast-completing tool calls
...
export function CollapsedReadSearchContent({
  ...
  if (verbose) {
    ...
    return <Box flexDirection="column">
        ...
        {message.hookInfos && message.hookInfos.length > 0 && <>
            <Text dimColor>
              {'  鈳? '}Ran {message.hookCount} PreToolUse{' '}
  ...
  if (isFullscreenEnvEnabled() && isActiveGroup) {
    ...
    if (elapsed !== undefined && elapsed >= 2) {
      ...
      shellProgressSuffix = lines > 0 ? ` (${time} 路 ${lines} ${lines === 1 ? 'line' : 'lines'})` : ` (${time})`;
    }
  }
  ...
  return <Box flexDirection="column" marginTop={1} backgroundColor={bg}>
    ...
    // Row layout: 5-wide gutter for 鈳? then a flex column for the text.
    // Ink's wrap stays inside the right column so continuation lines
    // indent under 鈳? MAX_HINT_CHARS in commandAsHint caps total at ~5 lines.
    <Box flexDirection="row">
          <Box width={5} flexShrink={0}>
            <Text dimColor>{'  鈳? '}</Text>
          </Box>
