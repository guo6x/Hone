 ...
 * their tool_results, and real user input. All assistant text is dropped ... * if the model forgets to call Brief, the user sees nothing for that turn.
...
export function filterForBriefTool<T extends {
  ...
  return messages.filter(msg => {
    ...
    if (msg.type === 'attachment') {
      ...
      // (query.ts mid-chain drain ->getQueuedCommandAttachments). Keep it ...      // it's what the user typed. commandMode === 'prompt' positively
...
type Props = {
  ...
  isBriefOnly?: boolean;
  /** Fullscreen-mode "鈹€鈹€鈹€ N new 鈹€鈹€鈹€" divider. Renders before the first
   ...
   *  Measured Mar 2026: 538-msg session, 20 slices ->鈭?5% plateau RSS. */
...
// anchor only advances when rendered count genuinely exceeds CAP+STEP ...// immune to length churn from grouping/compaction (CC-1174).
//
...
const MessagesImpl = ({
  ...
  // stays stable ...precompute the Set so the filter is O(k) not O(nxk) per chunk.
  ...
  const syntheticStreamingToolUseMessages = useMemo(() => streamingToolUsesWithoutInProgress.flatMap(streamingToolUse => {
    ...
    // fresh randomUUID ->unstable React keys ->component remounts ->    // Ink rendering corruption (overlapping text from stale DOM nodes).
  ...
  // messages + 4 filter/map passes = ~50ms alloc per scroll ->GC pressure ->  // 100-173ms stop-the-world pauses on the 1GB heap.
  const {
    ...
    // dropTextInBriefTurns should only trigger on SendUserMessage turns ...    // SendUserFile delivers a file without replacement text, so dropping
