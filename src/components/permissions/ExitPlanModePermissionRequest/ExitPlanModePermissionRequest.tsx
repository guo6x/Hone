...
export function autoNameSessionFromPlan(plan: string, setAppState: (updater: (prev: AppState) => AppState) => void, isClearContext: boolean): void {
  ...
  [createUserMessage({
    ...
    // On clear-context acceptance, regenerateSessionId() has run by now ...    // this intentionally names the NEW execution session. Do not "fix" by
...
export function ExitPlanModePermissionRequest({
  ...
  // Hide the Ultraplan button while a session is active or launching ...  // selecting it would dismiss the dialog and reject locally before
...
export function buildPlanApprovalOptions({
  ...
  if (showUltraplan) {
    options.push({
      label: 'No, refine with Ultraplan on Hone on the web',
      value: 'ultraplan'
    });
  }
  options.push({
