...
export function Stats(t0) {
  ...
  if ($[1] === Symbol.for("react.memo_cache_sentinel")) {
    t2 = <Box marginTop={1}><Spinner /><Text> Loading your Hone stats...</Text></Box>;
    $[1] = t2;
  } else {
    t2 = $[1];
  }
...
function StatsContent(t0) {
  ...
  if (allTimeResult.type === "empty") {
    ...
    if ($[15] === Symbol.for("react.memo_cache_sentinel")) {
      t7 = <Box marginTop={1}><Text color="warning">No stats available yet. Start using Hone!</Text></Box>;
      $[15] = t7;
    } else {
      t7 = $[15];
    }
  ...
  const t10 = copyStatus ? ` 路 ${copyStatus}` : "";
  let t11;
  if ($[29] !== t10) {
    t11 = <Box paddingLeft={2}><Text dimColor={true}>Esc to cancel 路 r to cycle dates 路 ctrl+s to copy{t10}</Text></Box>;
    $[29] = t10;
    $[30] = t11;
  } else {
    t11 = $[30];
...
function DateRangeSelector(t0) {
  ...
  let t1;
  if ($[0] !== dateRange) {
    t1 = DATE_RANGE_ORDER.map((range, i) => <Text key={range}>{i > 0 && <Text dimColor={true}> 路 </Text>}{range === dateRange ? <Text bold={true} color="claude">{DATE_RANGE_LABELS[range]}</Text> : <Text dimColor={true}>{DATE_RANGE_LABELS[range]}</Text>}</Text>);
    $[0] = dateRange;
    $[1] = t1;
  } else {
    t1 = $[1];
...
function ModelsTab(t0) {
  ...
  if ($[9] !== canScrollDown || $[10] !== canScrollUp || $[11] !== modelEntries || $[12] !== scrollOffset || $[13] !== showScrollHint) {
    t10 = showScrollHint && <Box marginTop={1}><Text color="subtle">{canScrollUp ? figures.arrowUp : " "}{" "}{canScrollDown ? figures.arrowDown : " "} {scrollOffset + 1}-{Math.min(scrollOffset + 4, modelEntries.length)} of{" "}{modelEntries.length} models (-{'>'}戔啌 to scroll)</Text></Box>;
    $[9] = canScrollDown;
    $[10] = canScrollUp;
    $[11] = modelEntries;
    $[12] = scrollOffset;
...
function ModelEntry(t0) {
  ...
  let t9;
  if ($[15] !== t7 || $[16] !== t8) {
    t9 = <Text color="subtle">{"  "}In: {t7} 路 Out:{" "}{t8}</Text>;
    $[15] = t7;
    $[16] = t8;
    $[17] = t9;
  } else {
...
function renderModelsToAnsi(stats: ClaudeCodeStats): string[] {
  ...
  if (chartOutput) {
    ...
    const legendLine = chartOutput.legend.map(item => `${item.coloredBullet} ${item.model}`).join(' 路 ');
    lines.push(legendLine);
    lines.push('');
  }

  // Summary
  lines.push(`${figures.star} Favorite: ${chalk.magenta.bold(renderModelName(favoriteModel?.[0] || ''))} 路 ${figures.circle} Total: ${chalk.magenta(formatNumber(totalTokens))} tokens`);
  lines.push('');
  ...
  for (const [model, usage] of topModels) {
    ...
    lines.push(chalk.dim(`  In: ${formatNumber(usage.inputTokens)} 路 Out: ${formatNumber(usage.outputTokens)}`));
  }
  return lines;
}
