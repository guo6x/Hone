...
export function HelpV2(t0) {
  ...
  if ($[8] === Symbol.for("react.memo_cache_sentinel")) {
    t4 = <Tab key="general" title="概述"><General /></Tab>;
    $[8] = t4;
  } else {
    t4 = $[8];
  }
  ...
  if ($[9] !== antOnlyCommands || $[10] !== builtinCommands || $[11] !== close || $[12] !== columns || $[13] !== customCommands || $[14] !== maxHeight) {
    ...
    if (false && antOnlyCommands.length > 0) {
      ...
      if ($[26] !== antOnlyCommands || $[27] !== close || $[28] !== columns || $[29] !== maxHeight) {
        t7 = <Tab key="ant-only" title="[仅内部]"><Commands commands={antOnlyCommands} maxHeight={maxHeight} columns={columns} title="浏览内部命令：" onCancel={close} /></Tab>;
        $[26] = antOnlyCommands;
        $[27] = close;
        $[28] = columns;
        $[29] = maxHeight;
  ...
  let t6;
  if ($[31] !== tabs) {
    t6 = <Tabs title={false ? "/help" : `Hone v${MACRO.VERSION}`} color="professionalBlue" defaultTab="general">{tabs}</Tabs>;
    $[31] = tabs;
    $[32] = t6;
  } else {
    t6 = $[32];
  ...
  if ($[34] !== dismissShortcut || $[35] !== exitState.keyName || $[36] !== exitState.pending) {
    t8 = <Box marginTop={1}><Text dimColor={true}>{exitState.pending ? <>再按 {exitState.keyName} 退出</> : <Text italic={true}>{dismissShortcut} 取消</Text>}</Text></Box>;
    $[34] = dismissShortcut;
