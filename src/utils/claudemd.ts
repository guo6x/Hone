 ...
 * 2. User memory (~/.hone/HONE.md, fallback ~/.claude/CLAUDE.md) - Private global instructions for all projects
 * 3. Project memory (HONE.md, .hone/HONE.md, .hone/rules/*.md in project roots; CLAUDE.md variants as fallback) - Instructions checked into the codebase
 * 4. Local memory (HONE.local.md in project roots, fallback CLAUDE.local.md) - Private project-specific instructions
 *
 ...
 * - HONE.md (priority) / CLAUDE.md (fallback), .hone/HONE.md / .claude/CLAUDE.md,
 *   and all .md files in .hone/rules/ + .claude/rules/ are checked in each directory
 *
...
function parseMemoryFileContent(
  ...
  // Only rebuild via tokens when a comment actually needs stripping ...  // marked normalises \r\n during lex, so round-tripping a CRLF file
...
/**
 * Try loading a Hone-branded memory file first, falling back to the
 * Claude-branded path if the Hone file doesn't exist. Implements the
 * "HONE.md first, CLAUDE.md fallback" compatibility strategy.
 */
async function processHoneOrClaudeFile(
  honePath: string,
  claudePath: string,
  type: MemoryType,
  processedPaths: Set<string>,
  ...
  const result = await processMemoryFile(
    honePath, type, processedPaths, includeExternal, depth, parent,
  )
  if (result.length > 0) return result
  return processMemoryFile(
    claudePath, type, processedPaths, includeExternal, depth, parent,
  )
}
...
export const getMemoryFiles = memoize(
  async (forceIncludeExternal: boolean = false): Promise<MemoryFileInfo[]> => {
    ...
    // Hone: try ~/.hone/HONE.md first, fall back to ~/.claude/CLAUDE.md
    if (isSettingSourceEnabled('userSettings')) {
      const userHoneMd = expandPath('~/.hone/HONE.md')
      const userClaudeMd = getMemoryPath('User')
      result.push(
        ...(await processHoneOrClaudeFile(
          userHoneMd,
          userClaudeMd,
          'User',
          processedPaths,
      ...
      // Process User rules from both ~/.hone/rules/ and ~/.claude/rules/
      const userHoneRulesDir = expandPath('~/.hone/rules')
      result.push(
        ...(await processMdRules({
          rulesDir: userHoneRulesDir,
          type: 'User',
          processedPaths,
    ...
    for (const dir of dirs.reverse()) {
      ...
      // Try reading Project memory files - Hone paths first, Claude fallback
      if (isSettingSourceEnabled('projectSettings') && !skipProject) {
        // HONE.md (priority) / CLAUDE.md (fallback)
        result.push(
          ...(await processHoneOrClaudeFile(
            join(dir, 'HONE.md'),
            join(dir, 'CLAUDE.md'),
        ...
        // .hone/HONE.md (priority) / .claude/CLAUDE.md (fallback)
        result.push(
          ...(await processHoneOrClaudeFile(
            join(dir, '.hone', 'HONE.md'),
        ...
        // Rules from both .hone/rules/ and .claude/rules/
        const honeRulesDir = join(dir, '.hone', 'rules')
        result.push(
          ...(await processMdRules({
            rulesDir: honeRulesDir,
            type: 'Project',
            processedPaths,
            includeExternal,
            conditionalRule: false,
      ...
      // HONE.local.md (priority) / CLAUDE.local.md (fallback)
      if (isSettingSourceEnabled('localSettings')) {
        result.push(
          ...(await processHoneOrClaudeFile(
            join(dir, 'HONE.local.md'),
    ...
    // Hone: HONE.md first, CLAUDE.md fallback
    ...
    if (false && teamMemPaths!.isTeamMemoryEnabled()) {
 ...
 * Loads HONE.md/CLAUDE.md, unconditional rules, and conditional rules for that directory.
 *
...
export async function getMemoryFilesForNestedDirectory(
  ...
  // Process project memory files — HONE.md first, CLAUDE.md fallback
  if (isSettingSourceEnabled('projectSettings')) {
    // HONE.md (priority) / CLAUDE.md (fallback)
    const projectResult = await processHoneOrClaudeFile(
      join(dir, 'HONE.md'),
      join(dir, 'CLAUDE.md'),
      'Project',
      processedPaths,
    ...
    // .hone/HONE.md (priority) / .claude/CLAUDE.md (fallback)
    const dotResult = await processHoneOrClaudeFile(
      join(dir, '.hone', 'HONE.md'),
      join(dir, '.claude', 'CLAUDE.md'),
      'Project',
      processedPaths,
    ...
    result.push(...dotResult)
  }

  // Process local memory file — HONE.local.md first, CLAUDE.local.md fallback
  if (isSettingSourceEnabled('localSettings')) {
    const localResult = await processHoneOrClaudeFile(
      join(dir, 'HONE.local.md'),
      join(dir, 'CLAUDE.local.md'),
      'Local',
      processedPaths,
  ...
  // Process unconditional rules from both .hone/rules/ and .claude/rules/
  const honeRulesDir = join(dir, '.hone', 'rules')
  ...
  // Process conditional rules from both .hone/rules/ and .claude/rules/
  result.push(
    ...(await processConditionedMdRules(
      targetPath,
      honeRulesDir,
...
export async function getConditionalRulesForCwdLevelDirectory(
  ...
  // Check both .hone/rules/ and .claude/rules/
  const honeRulesDir = join(dir, '.hone', 'rules')
  const claudeRulesDir = join(dir, '.claude', 'rules')
  const honeResult = await processConditionedMdRules(
    targetPath,
    honeRulesDir,
    'Project',
    processedPaths,
  ...
  const claudeResult = await processConditionedMdRules(
    targetPath,
    claudeRulesDir,
    'Project',
    processedPaths,
  ...
  return [...honeResult, ...claudeResult]
}
...
export function isMemoryFilePath(filePath: string): boolean {
  ...
  // HONE.md/HONE.local.md or CLAUDE.md/CLAUDE.local.md anywhere
  if (name === 'HONE.md' || name === 'HONE.local.md' || name === 'CLAUDE.md' || name === 'CLAUDE.local.md') {
    return true
  }

  // .md files in .hone/rules/ or .claude/rules/ directories
  if (name.endsWith('.md')) {
    const normalized = filePath.replace(/\\/g, '/')
    if (normalized.includes('/.hone/rules/') || normalized.includes('/.claude/rules/')) {
      return true
    }
  }
