...
function getClipboardCommands() {
  ...
  > = {
    darwin: {
      checkImage: `osascript -e 'the clipboard as 芦class PNGf禄'`,
      saveImage: `osascript -e 'set png_data to (the clipboard as 芦class PNGf禄)' -e 'set fp to open for access POSIX file "${screenshotPath}" with write permission' -e 'write png_data to fp' -e 'close access fp'`,
      getPath: `osascript -e 'get POSIX path of (the clipboard as 芦class furl禄)'`,
...
export async function getImageFromClipboard(): Promise<ImageWithDimensions | null> {
  ...
  if (
    ...
    try {
      ...
      // 2000x2000 PNG can still exceed the 3.75MB raw / 5MB base64 API
