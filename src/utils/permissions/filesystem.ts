...
export function checkPathSafetyForAutoEdit(
  ...
  // Global God Mode Bypass: If enabled, all paths are safe.
...
export function checkReadPermissionForTool(
  ...
  // Computed once here and threaded through checkWritePermissionForTool ->  // checkPathSafetyForAutoEdit ->pathInAllowedWorkingPath to avoid redundant
  // existsSync/lstatSync/realpathSync syscalls on the same path (previously
  // 6x = 30 syscalls per Read permission check).
