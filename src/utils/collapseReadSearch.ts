...
// ~5 lines 鑴?~60 cols. Generous static cap 閳?the renderer lets Ink wrap.
const MAX_HINT_CHARS = 300

/**
 * Format a bash command for the 閳?hint. Drops blank lines, collapses runs of
 * inline whitespace, then caps total length. Newlines are preserved so the
 * renderer can indent continuation lines under 閳?
 */
...
export function getToolSearchOrReadInfo(
  ...
  // REPL is absorbed silently 閳?its inner tool calls are emitted as virtual
  ...
  // as their own category 閳?"Ran N bash commands" instead of breaking the group.
  return {
    isCollapsible:
      isCollapsible ||
...
function scanBashResultForGitOps(
  ...
  // git push writes the ref update to stderr 閳?scan both streams.
...
type GroupAccumulator = {
  ...
  bashCount?: number
  // Bash tool_use_id 閳?command string, so tool results can be scanned for
...
function createCollapsedGroup(
  ...
  // Adding bash operation count on top would double-count 閳?e.g. Read(README.md)
  ...
  // memoryReadFilePaths 閳?readFilePaths (both populated from Read tool calls),
  // so this count is safe to subtract from totalReadCount at readCount below.
  // Absorbed relevant_memories attachments are NOT in readFilePaths 閳?added
  ...
  for (const msg of messages) {
    if (isCollapsibleToolUse(msg, tools)) {
      ...
      if (toolInfo.isMemoryWrite) {
        // Memory file write/edit 閳?check if it's team memory
        ...
        // Snip/ToolSearch absorbed silently 閳?no count, no summary text.
        ...
        // MCP search/read 閳?counted separately so the summary says
        ...
        // Non-search/read Bash command 閳?counted separately so the summary
        ...
        if (input?.command) {
          ...
          // for the human 閳?same trigger as the comment-as-label tool-use render).
          currentGroup.latestDisplayHint =
            extractBashCommentLabel(input.command) ??
            commandAsHint(input.command)
          // Remember tool_use_id 閳?command so the result (arriving next) can
        ...
        // Directory-listing bash commands (ls, tree, du) 閳?counted separately
        ...
        if (
          ...
          // Regular (non-memory) search 閳?collect pattern for display
        ...
        for (const filePath of filePaths) {
          ...
          if (false && teamMemOps?.isTeamMemFile(filePath)) {
            ...
            // Non-memory file read 閳?update display hint
      ...
      // 閳?block. Do NOT add paths to readFilePaths/memoryReadFilePaths 閳?      // that would poison the readOperationCount fallback (bash-only reads
      ...
      // 閳?Loaded lines cluster tightly instead of being split by the badge's marginTop.
      if (
        currentGroup.messages.length > 0 &&
 ...
 * @returns Summary text like "Searching for 3 patterns, reading 2 files, REPL'd 5 times閳?
 */
