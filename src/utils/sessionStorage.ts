...
export function getTranscriptPathForSession(sessionId: string): string {
  ...
  // track a sessionId->抪rojectDir map. Callers wanting a specific other
 ...
 * The internal chain (keep[i+1]->択eep[i]) is intact; only endpoints need
 * patching: head->抋nchor, and anchor's other children->抰ail. Anchor is the
...
function applyPreservedSegmentRelinks(
  ...
  // Validate tail->抙ead BEFORE mutating so malformed metadata is a true
  ...
  if (segIsLive) {
    ...
    if (!reachedHead) {
      // tail->抙ead walk broke ...a UUID in the preserved segment isn't in the
 ...
 *   1. Sibling assistant orphaned: walk goes prev->抋sstA->扵R_A->抧ext, drops asstB
 ...
 * write->抣oad round-trip drift ...the class of bugs where snip/compact/
...
export async function loadTranscriptFile(
  ...
  try {
    ...
    // append-only (parents before children), we record each progress->抪arent link
