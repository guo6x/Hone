 ...
 * enforceToolResultBudget makes the same choices ->same wire prefix -> * prompt cache hit. Mutating the clone does not affect the source.
 */
...
export async function enforceToolResultBudget(
  ...
  for (const candidates of candidatesByMessage) {
    ...
    if (fresh.length === 0) {
      // mustReapply/frozen are already in seenIds from their first pass ...      // re-adding is a no-op but keeps the invariant explicit.
    ...
    // concurrent reader (once subagents share state) ever sees X鈭坰eenIds
    // but X鈭塺eplacements, which would misclassify X as frozen and send
 ...
 *     Records for IDs not in messages (e.g. after compact) are skipped ... *     they're inert anyway.
