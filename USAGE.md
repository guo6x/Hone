...
- **Node.js** >= 22.5（运行时依赖内置 `node:sqlite`，低版本无法启动）
- **Bun** >= 1.3.5（依赖安装与 CLI 构建）
...
| **对话** | 与 Hone Gateway 24/7 对话，派发任务、网页自动化都在这里 |
| **工作台** | 多项目 PTY 终端，可专线进入任一 CLI 交互开发 |
| **盯盘** | 追踪股票等持仓与关注项，查看观测与复盘记录 |
...
网页自动化不再是单独的标签页 —— 直接在「对话」里说你想做什么即可，Gateway 会自动识别为浏览器任务。
